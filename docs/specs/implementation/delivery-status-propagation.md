...
Open items: none. The remediation review closed the three items the
qualification pass had opened: the received-report RFC 3464 profile accepts
the known fields of each group in any order and extension fields anywhere
inside their group, the propagation lane of the real Postfix qualification
passes, and the `cmd/dkim2-milter` ordinary-transit limitation is documented
as a deployment consequence rather than left as an open item.

Recorded limits that are not open items of this specification:

- This implementation signs an outgoing delivery-status notification only
  under the domain of the embedded original's highest signature
  (`delivery-status-signing.md`); it therefore cannot mint a notification
  ...
  design at `DSNEvidenceStageSigningDomain`; changing that is a signing-contract
  decision outside this specification.
- The Rspamd `dkim2.received_dsn_propagation` Policy attribute is opt-in
  (`nauthilus.received_dsn_attribute`, default `false`) because a Nauthilus
  policy with a closed `allowed_resource_attributes` list answers `403` to a
  request naming an attribute it has not admitted. The Nauthilus example
  policy for this verifier must admit the attribute with a string schema whose
  `max_length` is at least 30 before the option is enabled in production.
...
and lists the remaining fields of each group without an ordering
requirement;
...
| --- | --- | --- | --- | --- | --- |
| 01 | 2026-09-04T14:27:59+02:00 | 2026-09-04T15:06:01+02:00 | 0h 38m 02s | 0h 38m 02s | Plus review remediation 0h 11m 30s |
| 02 | 2026-09-04T18:09:21+02:00 | 2026-09-04T18:54:54+02:00 | 0h 45m 33s | 0h 45m 33s | Plus review remediation 0h 29m 37s |
| 03 | 2026-09-04T23:18:27+02:00 | 2026-09-04T23:42:00+02:00 | 0h 23m 33s | 0h 23m 33s | Plus joint remediation 0h 29m 07s |
| 04 | 2026-09-04T23:43:07+02:00 | 2026-09-05T00:17:22+02:00 | 0h 34m 15s | 0h 34m 15s | Plus continuation 0h 35m 33s and joint remediation 0h 29m 07s |
| 05 | 2026-09-05T05:48:51+02:00 | 2026-09-05T06:47:02+02:00 | 0h 58m 11s | 0h 58m 11s | Plus continuation 0h 35m 20s |
...
1. **Structure**: exact three-part RFC 6522 framing under the existing DSN
   parser limits; `message/delivery-status` parses under the received-report
   RFC 3464 profile. That profile keeps every field bound, unique, and
   unfolded except for the bounded continuation lines of `Remote-MTA` and
   `Diagnostic-Code`, keeps the required fields mandatory, but accepts the
   known per-message and per-recipient fields in any order and extension
   fields anywhere inside their group. RFC 3464 lists the fields of each
   group without an ordering requirement, and the most common generator,
   Postfix `bounce(8)`, emits its `X-<mail-name>-*` extensions before
   `Arrival-Date`; a profile that pins the ABNF sequence would classify every
   Postfix-generated notification as `malformed` and reject it in strict
   mode. The strict sequence profile remains the rule for the outgoing signing
   path, and the Postfix bounce wire profile is not selected here because the
   notification was generated by a foreign system.
