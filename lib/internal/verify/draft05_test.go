...
func TestVerifierAcceptsSHA512Only(t *testing.T) {
	message, parsed := draft05HashFixture(t, []canonical.HashAlgorithm{canonical.HashAlgorithmSHA512}, "")
...
func TestVerifierRejectsOneMismatchingSupportedTuple(t *testing.T) {
	message, parsed := draft05HashFixture(t, []canonical.HashAlgorithm{canonical.HashAlgorithmSHA256, canonical.HashAlgorithmSHA512}, canonical.HashAlgorithmSHA512)
	result, err := compareTargetHashes(mustCanonicalizer(t), message, parsed, Target{Sequence: 1, InstanceNumber: 1})
...
// draft05HashFixture constructs one parsed current Message-Instance with controlled supported tuples.
func draft05HashFixture(t *testing.T, algorithms []canonical.HashAlgorithm, mismatch canonical.HashAlgorithm) (rawmsg.Message, instance.MessageInstance) {
	t.Helper()
