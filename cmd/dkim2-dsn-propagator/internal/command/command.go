// Package command owns the dkim2-dsn-propagator Cobra surface and stable process exits.
package command
...
const (
  ...
  dkim2-dsn-propagator --version

Commands:
  serve    Run the DKIM2 delivery-status propagation adapter
...
Flags:
  ...
  -h, --help            help for dkim2-dsn-propagator
  -v, --version         version for dkim2-dsn-propagator
`
	commandShapeDiagnostic    = "dkim2-dsn-propagator: command usage error\n"
	commandRuntimeDiagnostic  = "dkim2-dsn-propagator: runtime failure\n"
...
var (
	errCommandShape   = errors.New("dkim2-dsn-propagator command shape failure")
	errCommandRuntime = errors.New("dkim2-dsn-propagator command runtime failure")
)
...
func newRootCommand(
	...
	root := &cobra.Command{
		Use:           "dkim2-dsn-propagator",
		Short:         "DKIM2 delivery-status propagation adapter",
	...
	root.SetVersionTemplate("dkim2-dsn-propagator {{.Version}}\n")
	root.SetOut(stdout)
	root.SetErr(stderr)
...
func newServeCommand(stderr io.Writer, deps commandDependencies) (*cobra.Command, error) {
	...
	serve := &cobra.Command{
		...
		Short:         "Run the DKIM2 delivery-status propagation adapter",
		Args:          cobra.NoArgs,
		SilenceUsage:  true,
		SilenceErrors: true,
