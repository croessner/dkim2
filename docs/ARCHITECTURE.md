...
| 0.1.0-draft | 2026-08-20 | Christian Roessner / Codex | Replaced the superseded Postfix full-envelope DSN handoff with the then-local bounce-only origin enum (superseded by upstream provenance on 2026-09-16). Embedded DSN verification omits only unavailable current-envelope observation while preserving cryptography, hashes, timestamp, custody, outer-recipient binding, and authenticated-domain authorization. |
...
- `sha256` and `sha512` are implemented; decoded values are exactly 32 and 64
  octets respectively, algorithm names are unique case-insensitively, unknown
  extension names remain forward-compatible, and verification requires every
  supplied supported tuple to agree.
...
- Encoded header-name object keys are lowercase. Parsing rejects decoded mixed
  or uppercase spellings, including escaped spellings, while message-header
...
- Selectors are unique case-insensitively within one field, and one signing
  algorithm may occur at most twice with different selectors. These bounds
  also apply to extension algorithm tokens and preserve the total-set limit.
...
attestation that the Postfix-only adapter received exact `bounce` origin from
`{postfix_internal_origin}`. The request carries no caller-selected fidelity. The embedded
...
5. The transport component is an MTA-neutral adapter, not a Milter: it
   ...
   required; the Postfix `{postfix_internal_origin}` enum remains the authority
...
- Implement SHA-256 and SHA-512 for Message-Instance body and header hashes.
  RSA-SHA256 and Ed25519-SHA256 signature inputs continue to use SHA-256.
- Accept and preserve syntactically valid unknown algorithm tokens at protocol
  parsing boundaries for draft forward compatibility, but never execute them.
  Ignore unknown Message-Instance hash tuples only as the active draft permits,
  require at least one supported tuple, and aggregate unknown signature
  algorithms as unsupported. Reject configuring an unknown cryptographic
  implementation unless it is explicitly enabled for experimental draft work.
...
The active compiler contract is exact Go 1.27.0 with
`GOEXPERIMENT=runtimesecret`. Historical delivery entries below retain their
original compiler identity and do not define an additional supported lane.
Make and direct test scripts set the experiment internally.
...
GOEXPERIMENT=runtimesecret go test -race ./lib/...
go vet ./lib/...
go vet ./cmd/dkim2d/...
...
| M25 - Outgoing delivery-status signing | Completed: byte-preserving RFC 6522 evidence, Draft Section 12.1 embedded verification, Section 12.1.2 local alignment, dedicated `delivery_status` profile use, route ticket, protected capability, `POST /v1/dsn/sign`, and the Postfix `{postfix_internal_origin}` adapter | measured in the ignored execution ledger | High; completed with independent review and live Postfix rollout evidence |
