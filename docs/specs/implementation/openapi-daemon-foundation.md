...
- RFC 9112 for HTTP/1.1 request-target, framing, connection management, and
  message reuse;
- the official `oapi-codegen` v2.8.0 documentation, including its OpenAPI 3.0
...
- it declares OpenAPI 3.1.0 although pinned `oapi-codegen` v2.8.0 supports
  OpenAPI 3.0;
...
- generates and commits reproducible server and client artifacts from one
  OpenAPI 3.0.3 source through pinned `oapi-codegen` v2.8.0; and
...
`oapi-codegen` v2.8.0 warns that 3.1 is unsupported and then silently emits
...
`github.com/oapi-codegen/oapi-codegen/v2 v2.8.0`. Both the tools module and
`cmd/dkim2d` directly pin `github.com/getkin/kin-openapi v0.135.0`, aligned
with the generator.
The v2.8.0 clients use `github.com/oapi-codegen/runtime v1.7.0` to decode the
declared response headers. The three generated-client owners pin that exact
direct dependency; the server-only `cmd/dkim2d` module and `lib/` do not.
Dependency guards require the generator, validator, and client-runtime pins,
require synchronized workspace module metadata, reject a runtime dependency in
`lib/` or the daemon, and reject an untagged middleware repository.
...
- the existing `github.com/valkey-io/valkey-go v1.0.77`.
...
dedicated tools module pins `oapi-codegen v2.8.0`. The YAML pin matches the
