...
- The real Postfix qualification of the propagation path is blocked. This
  implementation cannot produce the delivery-status notification shape the
  propagation route consumes, so the lane exists, runs, and reports its exact
  blocking stage instead of passing. See "Completion Evidence".
- The generic RFC 3464 group parser rejects the per-message field order real
  Postfix emits, so no notification generated by Postfix can be evaluated or
  propagated.
- `cmd/dkim2-milter` ordinary-transit mode cannot serve a forwarder that
  installs its own return path, because it sends the outgoing envelope as the
  inherited envelope.
...
| 07 | 2026-09-05T08:42:49+02:00 | 2026-09-05T09:51:30+02:00 | 1h 08m 41s | 1h 08m 41s | Continuation of a terminated pass; qualification lane blocked |
| 08 | | | | | Not started |
...
| --- | --- | --- | --- | --- |
| Scope | Received-DSN evaluation, rebuild, propagation route, adapter, projections, docs | All slices implemented; the qualification lane and the documentation set are complete | partial | The propagation lane cannot pass end to end; see "Completion Evidence" |
| Behavior | Draft-06 Sections 8.7, 9.3, 11.4, 12, 12.1.1, 12.1.2 and RFC 6522/3464/3834/2033 semantics are explicit and tested | Covered by unit, golden-vector, and fuzz coverage in `lib` and by daemon and adapter tests | partial | Real-MTA behavior exposed two RFC 3464 and envelope-inheritance gaps recorded as findings |
...
| Tests | Unit, golden, fuzz, race, privacy, daemon, adapter, client, and real-MTA evidence exist | Present except the propagation real-MTA evidence and the Rspamd policy end-to-end run | partial | Both blocked, with exact reasons recorded |
| Documentation | Architecture, operator, reference, provisioning, and limitation documents agree | Verified against the implementation; four documentation claims that the code does not honor were corrected | done | `tools/check-operator-docs.sh` refreshed and its inert refutations fixed |
