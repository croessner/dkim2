...
// RecordStarted records the quiescent active lifecycle transition.
func (r *Runtime) RecordStarted() {
	_ = r.Activate()
}

// Activate enables the central logger and registry without a listener or goroutine.
...
// RecordStopped withdraws readiness and records the stopped transition once.
func (r *Runtime) RecordStopped() {
	contextWithTimeout, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	_ = r.Stop(contextWithTimeout)
}
...
// RecordConfigAccepted records one redacted effective configuration outcome.
func (r *Runtime) RecordConfigAccepted(mode string, failOpen bool) error {
	if r == nil {
		return errConfiguration
	}
	r.mu.Lock()
	...
	if failOpen {
		return r.emit(slog.LevelWarn, eventConfigAccepted, true, attributes...)
	}
	return r.emit(slog.LevelInfo, eventConfigAccepted, false, attributes...)
}
