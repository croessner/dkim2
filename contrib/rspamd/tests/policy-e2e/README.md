...
| Received delivery-status projection | The producer returns the optional `delivery_status` member. Rspamd publishes all six projected zero-score `DKIM2_DSN_*` observations on the scanned message and, with `nauthilus.received_dsn_attribute = true`, carries `dkim2.received_dsn_propagation` to Policy, while disposition, replay, and action semantics stay unchanged. With the attribute disabled the symbols are still proven and the attribute is asserted absent. |
...
exactly when the served producer response carries `delivery_status` and the
Rspamd option `nauthilus.received_dsn_attribute` is `true`, and absent
otherwise. The harness enables the option explicitly in
`rspamd/local.d/dkim2.conf`; `POLICY_E2E_RECEIVED_DSN_ATTRIBUTE=false`
rewrites it to `false` in the runtime copy of that directory and asserts the
attribute absent. The enabled variant requires a Nauthilus policy whose
`allowed_resource_attributes` admits the attribute with a string schema of
`max_length` at least 30; against a policy that has not admitted it, Nauthilus
answers `403` and the received-DSN scenario fails at Policy admission.
