...
func descriptorAccessFingerprint(fd int, directory bool, acceptedMode uint32) ([32]byte, error) {
	...
	return linuxAccessFingerprint(filesystemType, directory, acceptedMode,
		func(destination []byte) (int, error) { return unix.Flistxattr(fd, destination) },
		func(name string, destination []byte) (int, error) {
			return unix.Fgetxattr(fd, name, destination)
		},
	)
}

// SocketAccessFingerprint validates and fingerprints one no-follow socket access model.
func SocketAccessFingerprint(path string, acceptedMode uint32) ([32]byte, error) {
	...
	return linuxAccessFingerprint(filesystemType, false, acceptedMode,
		func(destination []byte) (int, error) { return unix.Listxattr(path, destination) },
		func(name string, destination []byte) (int, error) {
			return unix.Getxattr(path, name, destination)
		},
	)
}

// linuxAccessFingerprint validates and hashes one stable filesystem access model.
func linuxAccessFingerprint(
	filesystemType int64,
	directory bool,
	acceptedMode uint32,
	list func([]byte) (int, error),
	get func(string, []byte) (int, error),
) ([32]byte, error) {
	names, err := readLinuxXattrNamesWith(list)
	if err != nil {
		return [32]byte{}, err
	}
	...
	access, accessPresent, err := readLinuxACLXattrWith(func(destination []byte) (int, error) {
		return get(linuxACLAccessName, destination)
	})
	if err != nil {
		return [32]byte{}, err
	}
	defaultACL, defaultPresent, err := readLinuxACLXattrWith(func(destination []byte) (int, error) {
		return get(linuxACLDefaultName, destination)
	})
	if err != nil {
		return [32]byte{}, err
	}
	...
	hash := sha256.New()
	_, _ = hash.Write([]byte("dkim2-exim-securefile-linux-access-v1\x00"))
	var fixed [13]byte
