...
const failedSelector = "failed.example"
...
func TestSignatureResultsPreserveOccurrenceOrder(t *testing.T) {
	...
	sets := []verify.SignatureSetResult{
		...
		{Index: 1, Selector: failedSelector, Algorithm: verify.AlgorithmRSASHA256, Status: verify.SignatureSetStatusFail, KeyStatus: verify.KeyStatusFound},
	}
	...
	if got[0].Selector != "" || got[1].Selector != failedSelector {
		t.Fatalf("signature selectors = %q, %q", got[0].Selector, got[1].Selector)
	}
}

// TestDraft06ProtocolInfractionsMapToDistinctPermanentReasons proves the four Section 11.2 failures stay distinct.
func TestDraft06ProtocolInfractionsMapToDistinctPermanentReasons(t *testing.T) {
	tests := []struct {
