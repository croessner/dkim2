...
// main validates one default and one explicit demo rendering.
func main() {
	...
	flag.Parse()
	if flag.NArg() != 0 || defaultPath == "" || demoPath == "" || root == "" {
		fmt.Fprintln(os.Stderr, "usage: deploymentpolicy -default <json> -demo <json> -root <root>")
		os.Exit(2)
	}
	if err := run(defaultPath, demoPath, root); err != nil {
...
// run validates both rendered topologies and checked Postfix configuration.
func run(defaultPath, demoPath, root string) error {
...
// validatePostfix freezes Milter routes, callback policy, and reserved identities.
func validatePostfix(root string) error {
	mainCF, err := artifactpath.ReadFile(
		root, "deployments/postfix-compose/postfix/main.cf", maximumConfigBytes,
	)
	if err != nil {
		return err
	}
	masterCF, err := artifactpath.ReadFile(
		root, "deployments/postfix-compose/postfix/master.cf", maximumConfigBytes,
	)
	if err != nil {
		return err
	}
	if string(mainCF) != expectedMainCF || string(masterCF) != expectedMasterCF {
		return errPolicy
	}
	return nil
}
