...
// TestDatasourceRootPoolOwnsCertificateBytes proves callback cleanup cannot corrupt retained trust.
func TestDatasourceRootPoolOwnsCertificateBytes(t *testing.T) {
	t.Parallel()

	const serverName = "ldap.internal.example"
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "datasource-root"},
	...
	callbackDER := bytes.Clone(der)
	roots, err := datasourceRootPool([][]byte{callbackDER})
	if err != nil {
		t.Fatal("construct datasource root pool")
	}
	clear(callbackDER)
	if _, err := candidate.Verify(x509.VerifyOptions{
		...
		t.Fatal("callback cleanup corrupted retained datasource trust")
	}
}
