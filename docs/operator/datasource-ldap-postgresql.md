# LDAP, PostgreSQL, MySQL, And MariaDB Datasources

The daemon supports one exact signing datasource backend at a time:
`flat_file`, `ldap`, `postgresql`, or `mysql`. All three network provider
classes use the native
...
Install MySQL or MariaDB with
`contrib/schema/mysql/001_dkim2_datasource.sql` in each dedicated datasource
database. The same DDL is qualified against MySQL 8.4 and MariaDB 10.11. It
uses InnoDB, exact binary collations, foreign keys, immutable-generation
triggers, and one permanent publication-lock row. Create separate TLS-required
login principals. Grant the runtime principal `SELECT` on the seven dataset
tables only. Grant the publisher `SELECT` and `INSERT` on the dataset tables,
`UPDATE` only on `dkim2_dataset_generations`,
`dkim2_current_generation`, and `dkim2_publication_lock`; the last grant is
required by MySQL-family locking reads. Grant neither principal `DELETE`,
DDL, `FILE`, local-infile, global, or administrative privileges.

All servers require verified TLS and one configured endpoint. LDAP supports
...
A MySQL or MariaDB signing selection uses the common `mysql` backend:
...
signing:
  ...
  mysql:
    ...
    ca_file: /secure/0123456789abcdef0123456789abcdef/mysql-ca.pem
    database: dkim2
    user: dkim2_runtime
    password_file: /secure/0123456789abcdef0123456789abcdef/mysql-password
    page_size: 128
    load_deadline: 5s
    max_connections: 2
    idle_connections: 1
...
The daemon constructs the driver configuration from these typed fields. It
does not accept a DSN. TLS 1.2 or newer, CA validation, and exact server-name
validation are mandatory. Unix sockets, multi-host failover, multi-statements,
local infile, cleartext-password fallback, old-password fallback, and
interpolated parameters are disabled.
...
PostgreSQL, MySQL, or MariaDB is rejected.
...
pointer. MySQL and MariaDB additionally serialize publication through their
permanent singleton lock row before proving emptiness or the expected current
generation. A missing current pointer in a nonempty backend is corruption, not a
...
Back up committed datasource generations, including `ou=key-material`,
`dkim2_datasource.key_material`, or `dkim2_key_material`, and the singleton current reference as key
...
- MySQL/MariaDB load rejection: check TLS-required accounts, exact CA and
  server identity, the dedicated database, runtime `SELECT` grants, session
  `REPEATABLE READ` plus `READ ONLY`, InnoDB tables, triggers, and the singleton
  current row.
...
Run `make check-datasource-schema`, `make check-datasource-postgresql`,
`make check-datasource-mysql`, and `make guardrails` after schema,
...
digest-pinned disposable OpenLDAP, PostgreSQL, MySQL, and MariaDB images; an unavailable
