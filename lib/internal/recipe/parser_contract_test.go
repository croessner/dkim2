...
func TestParserEveryOwnedLimitHasExactAndOneOverProof(t *testing.T) {
	...
	cases := []limitCase{
		...
		{"header names", testHeaderRemovalRecipe, testTwoHeaderRecipe, limitNameMaxHeaderNames, func(l *Limits) { l.MaxHeaderNames = 1 }},
		{"header name bytes", testHeaderRemovalRecipe, `{"h":{"AA":[]}}`, limitNameMaxHeaderNameBytes, func(l *Limits) { l.MaxHeaderNameBytes = 1; l.MaxTotalHeaderNameBytes = 2 }},
		...
		{"total steps", `{"h":{"A":[{"d":["x"]}]},"b":[{"d":["y"]}]}`, `{"h":{"A":[{"d":["x"]},{"d":["z"]}]},"b":[{"d":["y"]}]}`, limitNameMaxTotalSteps, func(l *Limits) { l.MaxTotalSteps = 2; l.MaxStepsPerHeader = 2; l.MaxBodySteps = 2 }},
