...
// ClientConfig contains normalized production application-client inputs.
...
// AuditorConfig contains only protected ephemeral-auditor credentials.
...
func validateClientConfig(config ClientConfig) (validatedClientConfig, error) {
	if config.Topology != TopologyStandalonePrimary ||
		config.Database != 0 ||
		config.ClientName != "" ||
		!config.DisableCache ||
		!config.DisableRetry ||
		config.Draft != dkim2.DraftIdentifier ||
		config.Algorithm != dkim2.ReplayKeyAlgorithm ||
		config.Namespace != replayNamespace ||
		config.Epoch == 0 ||
		config.MinimumRetention != time.Second ||
		config.MaximumRetention != 30*24*time.Hour {
		return validatedClientConfig{}, dkim2.NewReplayError(dkim2.ReplayErrorMisconfigured)
	}
	resolvedLimits, err := resolveAdmissionLimits(config.Limits)
	if err != nil {
		return validatedClientConfig{}, err
	}
	if !validCanonicalEndpoint(config.Endpoint) ||
		!validTLSServerName(config.TLSServerName) ||
		!validUsername(config.Username) ||
		len(config.Password) < 1 || len(config.Password) > 1024 ||
		config.DialTimeout < 100*time.Millisecond ||
		config.DialTimeout > 30*time.Second ||
		config.TCPKeepAlive < time.Second ||
		config.TCPKeepAlive > 5*time.Minute ||
		config.ConnWriteTimeout < 100*time.Millisecond ||
		config.ConnWriteTimeout > 30*time.Second {
		return validatedClientConfig{}, dkim2.NewReplayError(dkim2.ReplayErrorMisconfigured)
	}
	roots, err := cloneAndValidateRoots(config.RootCertificatesDER)
	if err != nil {
		return validatedClientConfig{}, err
	}
	...
	return validatedClientConfig{
		...
		connWriteTimeout:    config.ConnWriteTimeout,
		epoch:               config.Epoch,
...
func validateAuditorConfig(config AuditorConfig) (validatedAuditorConfig, error) {
	if !validUsername(config.Username) || len(config.Password) < 1 || len(config.Password) > 1024 {
