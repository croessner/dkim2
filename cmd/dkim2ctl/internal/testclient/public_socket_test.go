...
func (s *conformanceService) writeError(
	...
	body := `{"api_version":"v1","draft":"draft-ietf-dkim-dkim2-spec-06","code":"` +
...
const processReportResponse = `{"api_version":"v1","draft":"draft-ietf-dkim-dkim2-spec-06","authentication":{"state":"PASS","primary_reason":"none"},"verification":{"state":"PASS","primary_reason":"none","scope":"chain","historical_content":"complete","historical_signatures":"complete","custody_structure":"not_present","checks":[{"class":"protocol","reason":"none"}],"signature_sets":[],"target":{"sequence":"1","instance":"1"}},"verifier_projection":{"schema":"dkim2.verifier-projection.v1","draft":"draft-ietf-dkim-dkim2-spec-06","binding_algorithm":"sha-256","binding":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=","hops":[{"sequence":"1","message_instance":"1","hop_binding":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=","signer_domain":"example.test","signature_algorithms":["rsa-sha256"],"signature_state":"pass","custody_transition":"origin","do_not_modify":false,"do_not_explode":false,"feedback":false,"feed_here":false,"exploded":false,"recipe_mode":"unchanged","recipe_has_header_changes":false,"recipe_body_mode":"absent","recipe_digest":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=","change_classes":[],"affected_headers":[],"history_header_state":"matched","history_body_state":"matched","body_availability":"known","change_count":0,"affected_header_count":0}]},"policy":{"mode":"strict","verdict":"accept","primary_reason":"protocol_pass","do_not_modify":"not_requested","do_not_explode":"not_requested","dns_testing_effective":false,"feedback":{"requested":false,"relay_required":false,"history_coverage":"complete"},"findings":[{"reason":"protocol_pass","severity":"info"}]},"replay":{"class":"first_seen"},"disposition":"accept","actions":[{"type":"add_header","name":"Authentication-Results","value":"mx.example.test; dkim2=pass"}]}`
const signContinueResponse = `{"api_version":"v1","draft":"draft-ietf-dkim-dkim2-spec-06","operation":"sign","result":"pass","disposition":"continue","actions":[]}`
const reviseUnchangedResponse = `{"api_version":"v1","draft":"draft-ietf-dkim-dkim2-spec-06","operation":"revise","result":"pass","disposition":"accept","actions":[{"type":"add_header","name":"DKIM2-Signature","value":"v=1; a=ed25519-sha256; d=example.test; s=test; b=unchanged"}]}`
const reviseChangedResponse = `{"api_version":"v1","draft":"draft-ietf-dkim-dkim2-spec-06","operation":"revise","result":"pass","disposition":"accept","actions":[{"type":"add_header","name":"Message-Instance","value":"v=1; i=2; h=sha256:synthetic"},{"type":"add_header","name":"DKIM2-Signature","value":"v=1; a=ed25519-sha256; d=example.test; s=test; b=changed"}]}`

// TestProcessReportFixtureUsesCompletedOriginPolicy verifies the frozen daemon response is internally coherent.
func TestProcessReportFixtureUsesCompletedOriginPolicy(t *testing.T) {
	var response generated.ProcessResponse
	if err := json.Unmarshal([]byte(processReportResponse), &response); err != nil {
		t.Fatalf("process report fixture error = %v", err)
	}
	if response.Verification.Scope != generated.Chain || response.Verification.HistoricalContent != generated.VerificationResultHistoricalContentComplete ||
		response.Policy.DoNotModify != generated.PolicyResultDoNotModifyNotRequested ||
		response.Policy.DoNotExplode != generated.PolicyResultDoNotExplodeNotRequested ||
		response.Policy.Feedback.HistoryCoverage != generated.PolicyFeedbackHistoryCoverageComplete {
		t.Fatalf("process report fixture has incoherent completed-origin policy: %#v", response.Policy)
	}
}
